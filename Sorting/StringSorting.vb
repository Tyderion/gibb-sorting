Public Class StringSorting 'Rename??
    ...
    Public Sub Insertionsort()
        ...
        Do
            ...
            For i = intLeft + 1 To intLength - 1
                If StrComp(maskUmlaute(strWords(i)), maskUmlaute(strWords(intMax)), optionCompare()) = -1 Then
                    intMax = i
                End If
    ...
    Public Sub BubbleSort()
        ...
        Do
            swapped = False
            For i = 0 To length - 2
                If StrComp(maskUmlaute(strWords(i)), maskUmlaute(strWords(i + 1)), optionCompare()) = 1 Then
                    SwapValues(i, i + 1)
                    swapped = True
                End If
                If Not blnAlive Then ' Make it correctly quit when quitting :)
    ...
    Public Sub Quicksort(Optional ByVal intLeft As Integer = 0, Optional ByVal intRight As Integer = Int32.MaxValue)
        ...
        While i <= j
            ...
            While StrComp(maskUmlaute(strWords(i)), maskUmlaute(x), optionCompare()) = -1
                i += 1
            End While
            While StrComp(maskUmlaute(x), maskUmlaute(strWords(j)), optionCompare()) = -1
                j -= 1
            End While
            If i <= j Then
                SwapValues(i, j)
